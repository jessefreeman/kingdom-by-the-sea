(function runTests(){
  ...
  function tests(){
    ...
    try{
      ...
      as(afford({G:1})===true,'afford G1'); as(afford({G:999})===false,'afford G999');
      ...
      state.size={w:1,h:1,t:snap.size.t}; state.map=[{type:T.GRASS,disc:true,upg:null}]; state.gold=5; state.wood=5; state.actions=2; startUpgrade(0,0,state.map[0],{to:T.FARM,cost:{W:1},duration:1}); as(!!state.map[0].upg,'upgrade set'); as(state.actions===1,'action used'); as(state.map[0].upg.left===1,'1t upgrade not decremented');
      const tl=c=>label(c); as(tl({type:T.WATER,disc:false})==='','no label water'); as(tl({type:T.GRASS,disc:false})==='?','label hidden'); as(tl({type:T.GRASS,disc:true})==='G','label grass');
      state.size={w:2,h:1,t:snap.size.t}; state.map=[cell(T.GRASS),cell(T.GRASS)]; state.map[0].disc=true; state.actions=1; state.people=2; state.food=2; state.riskRng=()=>0.99; as(K.canExplore(1,0)===true,'can explore adj'); K.explore(1,0); as(state.map[1].disc===true,'explored reveal'); as(state.food===1,'food cost');
      state.size={w:2,h:1,t:snap.size.t}; state.map=[cell(T.GRASS),cell(T.GRASS)]; state.map[0].disc=true; state.actions=1; state.people=2; state.food=2; state.riskRng=()=>0.0; K.explore(1,0); as(state.people===1,'risk loss');
      state.size={w:1,h:1,t:snap.size.t}; state.map=[{type:T.HOUSE,disc:true,upg:null}]; state.gold=10; state.wood=0; state.actions=2; startUpgrade(0,0,state.map[0],{to:T.MANSION,cost:{G:2},duration:2}); as(state.map[0].upg.left===1&&state.map[0].upg.total===2&&state.map[0].upg.prog===1,'multi-step shows first step');
      state.size={w:2,h:1,t:snap.size.t}; state.map=[cell(T.GRASS),cell(T.GRASS)]; state.map[0].disc=true; state.actions=1; state.people=2; state.food=0; state.riskRng=()=>0.99; as(K.canExplore(1,0)===false,'cannot explore when F:0');
      state.size={w:3,h:1,t:snap.size.t}; state.map=[cell(T.GRASS),cell(T.GRASS),cell(T.GRASS)]; state.map[0].disc=true; state.actions=2; state.people=2; state.food=3; state.year=2; state.riskRng=()=>0.2; K.explore(1,0); as(state.people===2,'early risk lowered (no loss at 0.2)'); as(state.map[2].disc===true,'explore expands FoW radius');
      state.size={w:2,h:2,t:snap.size.t}; state.map=[cell(T.GRASS),cell(T.GRASS),cell(T.GRASS),cell(T.GRASS)]; state.map[idx(0,0)].disc=true; state.actions=1; state.people=2; state.food=2; state.riskRng=()=>0.99; as(K.canExplore(1,1)===false,'diagonal explore blocked'); K.explore(1,1); as(state.map[idx(1,1)].disc===false,'diagonal reveal blocked');
      as(SPEC[T.GRASS][0].duration===1,'farm duration 1t'); as(SPEC[T.HILL][0].duration===1,'mine duration 1t');
      // Treasure rarity
      const saveRng=state.rng; state.rng=rng32(12345); let tCount=0; for(let i=0;i<50;i++){const dd={G:0,F:0,W:0,P:0,events:[]}; randomEvent(dd); if(dd.events.some(s=>String(s).toLowerCase().includes('treasure'))) tCount++; } as(tCount<20,'treasure not too frequent'); state.rng=saveRng;
      as(!!SPEC[T.BURNT]&&SPEC[T.BURNT][0].to===T.GRASS&&SPEC[T.BURNT][0].cost.W===1&&SPEC[T.BURNT][0].duration===1,'Burnt→Grass W1 1t');
      as(!!SPEC[T.RUBBLE]&&SPEC[T.RUBBLE][0].to===T.HUT&&SPEC[T.RUBBLE][0].cost.W===1&&SPEC[T.RUBBLE][0].duration===1,'Rubble→Hut W1 1t');
      const infoClean=K.tileInfo({type:T.GRASS,upg:null}); as(infoClean.status==='—','idle tile status'); const s1=K.tileInfo({type:T.BURNT,upg:{to:T.GRASS,left:1}}).status; as(s1.includes('CLEAR')&&s1.includes('1t'),'burnt shows CLEAR'); const s2=K.tileInfo({type:T.RUBBLE,upg:{to:T.HUT,left:2}}).status; as(s2.includes('REBUILD')&&s2.includes('2t'),'rubble shows REBUILD');
      try{K.summary({G:0,F:0,W:0,P:0,events:['Test event A','Test event B']}); const ok=document.getElementById('ok'); if(ok) ok.click()}catch(e){throw new Error('summary with events failed')}
      state.size={w:3,h:3,t:snap.size.t}; state.map=[cell(T.GRASS),cell(T.GRASS),cell(T.GRASS),cell(T.GRASS),cell(T.GRASS),cell(T.GRASS),cell(T.GRASS),cell(T.GRASS),cell(T.GRASS)]; reveal(0,0,1); as(state.map[idx(0,0)].disc===true&&state.map[idx(1,1)].disc===true,'reveal edges safe'); reveal(1,1,1); as(state.map[idx(0,0)].disc===true&&state.map[idx(2,2)].disc===true,'reveal radius works');
      state.size={w:3,h:1,t:snap.size.t}; state.map=[cell(T.GRASS),cell(T.GRASS),cell(T.GRASS)]; state.map[1].disc=true; state.actions=2; state.gold=0; state.wood=1; startUpgrade(1,0,state.map[1],{to:T.FARM,cost:{W:1},duration:2}); as(state.map[0].disc===true&&state.map[2].disc===true,'upgrade reveal immediate');
      state.size={w:1,h:1,t:snap.size.t}; state.map=[cell(T.FARM)]; state.map[0].disc=true; state.gold=1; state.wood=1; state.people=1; state.actions=1; state.year=1; startUpgrade(0,0,state.map[0],SPEC[T.FARM][0]); endTurn(); as(state.map[0].type===T.HOUSE,'farm converts to house'); as(state.people===2,'+1 person from conversion');
      ...
      state.size={w:1,h:1,t:snap.size.t}; state.map=[cell(T.FARM)]; state.map[0].disc=true; state.people=0; state.actions=0; state.food=0; state.year=10; endTurn(); as(state.food===0,'no worker, no farm yield'); state.map[0].wrk=1; endTurn(); as(state.food===2,'worker yields food (2F)');
      state.size={w:1,h:1,t:snap.size.t}; state.map=[cell(T.FARM)]; state.map[0].disc=true; state.map[0].wrk=1; state.people=3; state.actions=0; state.year=5; endTurn(); as(state.actions===2,'actions reduced by farm workers at start of turn');
    ...
    finally{Object.assign(state,snap); state.size=snap.size; state.map=snap.map; resize(); hud(); draw();}
  }

  // Run tests once on load, then show start screen
